...
It still has to *look* like an application, and that is a separate problem from serving the page. There are two answers, tried in order.

**Windows draws its own window.** `internal/platform.RunNativeWindow` hosts the WebView2 runtime — part of the operating system — in a window this program creates and owns. No browser process, no browser profile, this program's icon in the taskbar and Alt-Tab. It is reachable from Go without cgo, so it costs neither `CGO_ENABLED=0` nor cross-compilation from one Linux machine, which is why Windows gets it and the others do not.

Two properties of that path are load-bearing and are enforced in code rather than assumed. The window blocks on a locked OS thread, because a Win32 message loop must; the HTTP server therefore moves behind it, and the window closing is what cancels the server's context. And `WebView2Loader.dll` is shipped as a file beside the executable, loaded by absolute path before the library gets a chance to look for it. That is not packaging taste: when the loader is not found on disk, the library maps an embedded copy into the process without the operating system's loader. Reflective loading is a technique this project will not use — it is what security tooling flags, and a program that reports on a machine's security posture has to be defensible to the same tooling. `NativeWindowAvailable` therefore checks the file and the runtime registry key *before* the library is touched, and a Windows test asserts that a binary with no loader beside it refuses the native window rather than reaching the fallback.

Being a windowed program has one cost that has to be paid explicitly rather than inherited. A process with no console that starts a console program gets that child a brand new console window — so every check that runs a system tool would flash a black window and take it away again, dozens of times per run. `platform.NoConsoleWindow` sets `CREATE_NO_WINDOW` on every child the agent starts, which Windows ignores for programs that are not console programs. The tools the checks run get `platform.HideConsoleTool`, which adds `STARTF_USESHOWWINDOW`/`SW_HIDE` on top: the documentation says the creation flag alone is enough, a real machine said otherwise, and Windows has more than one route to a console window — a redirected default terminal application among them. `SW_HIDE` is applied only there, never through `NoConsoleWindow`, because it hides a windowed program's window too and would hide the browser the fallback path is trying to open.

The other cost is that the Windows agent is two files rather than one. The installer hides it; the archive explains it; and an executable separated from its loader degrades to the path below rather than breaking.

**Everywhere else borrows a browser's window.** `internal/platform.OpenAppWindow` asks an already-installed Chromium-family browser for an app window — `--app=`, the window those browsers use for installed web apps — which has no address bar, no tab strip and its own entry in the taskbar or dock. The browser is found in a compiled-in table of the paths these programs install themselves to, because neither Windows nor macOS puts them on `PATH`; the URL is the agent's own loopback address, checked before anything launches; no shell is involved either way.

The alternative on those platforms is WKWebView through the Objective-C runtime, or WebKitGTK. Both need cgo, a platform SDK at build time — a macOS SDK is not something a Linux machine can lawfully or reproducibly carry — and on Linux a library the user must already have installed. That would cost `CGO_ENABLED=0`, cross-compilation of nine targets from one machine, the single static file, and the byte-for-byte reproducibility gate, for a window that looks the same as the one `--app=` gives for the price of one exec. It is a decision that can be revisited, and revisiting it means accepting a macOS build runner and a runtime dependency on Linux; it is not a gap that better code closes. Where no such browser exists the interface opens as an ordinary tab: a worse window, never a missing one.
