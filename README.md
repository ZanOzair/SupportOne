...
**Phase 2 of 6 — the snapshot, plus repairs that can be undone.** Twelve read-only checks, a local web interface, HTML and JSON reports, redaction before anything is saved — and now three fixes and two guided walkthroughs, each behind a consent gate that will not act on a change it did not describe.

The offline explainer, the optional AI assistant, the fleet server and remote help are later phases and are **not** in this build. The agent says so rather than implying otherwise.
...
| 3 | Offline explainer, optional LLM assistant, performance and backup analysis | Next |
